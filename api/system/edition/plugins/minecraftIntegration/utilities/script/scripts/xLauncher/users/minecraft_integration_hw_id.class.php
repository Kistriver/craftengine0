...
class minecraft_integration_hw_id implements userInterface
{
	...
	{
		$id = intval($id);
		$qr = $this->core->mysql->query("SELECT minecraft_integration_hw_id FROM users WHERE id='$id'");

		if($this->core->mysql->rows($qr)==0)return false;

		$fr = $this->core->mysql->fetch();

		return $fr['minecraft_integration_hw_id'];
	}

	public function getPropertyByValue($value)
	{
		$value = $this->core->sanString($value);
		$qr = $this->core->mysql->query("SELECT id FROM users WHERE minecraft_integration_hw_id='$value'");
	...
	public function setProperty($id,$value)
	{
		...
		$value = trim($value);
		$value = $this->core->sanString($value);
		$qr = $this->core->mysql->query("UPDATE users SET minecraft_integration_hw_id='$value' WHERE id='$id'");
