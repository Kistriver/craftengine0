...
class views implements articleInterface
{
	...
	public function install()
	{
		if($this->core->mysql->rows(
			$this->core->mysql->query("SHOW COLUMNS FROM articles LIKE 'views'")
		)==0)
		{
			$qr = $this->core->mysql->query("ALTER TABLE articles ADD views INT(10)");
			if(!$qr)return false;
		}
		return true;
	}
	...
	{
		$id = intval($id);
		$qr = $this->core->mysql->query("SELECT views FROM articles WHERE id='$id'");
	...
	public function setProperty($id,$value)
	{
		...
		$qr = $this->core->mysql->query("UPDATE articles SET views='$value' WHERE id='$id'");
	...
	public function canSetProperty($id,$uid)
	{
		...
		$r = $uc->permission->canDo('articles_edit',$uid,array('id'=>$id,'property'=>'views'));
		return $r;
	}

	public function newPost($id,$value)
	{
		if(!$this->setProperty($id,0))return false;
		return true;
	}
}