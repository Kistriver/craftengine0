...
class upload
{
	public function __construct($core)
	{
		...
		$r = $core->mysql->query("SELECT * FROM system_uploads WHERE hash='$hash' AND ip='$ip' AND time='$time_was' LIMIT 0,1");
		...
		if($core->mysql->rows($r)==1)
		{
			...
			{
				$core->mysql->query("DELETE FROM system_uploads WHERE hash='$hash' AND ip='$ip' AND time='$time_was'");
