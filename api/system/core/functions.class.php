...
class functions
{
	...
	public function arrayKeyExists($array,$key,$none)
	{
		if(is_array($key))
		{
			if(!key_exists(key($key),$array))return false;
			return $this->arrayKeyExists($array[key($key)],$key[key($key)],$none);
		}
		else
		{
			return key_exists($key,$array)?$array[$key]:$none;
		}
	}
	...
	{
		//if(!empty($this->core->getParams()['sid']))$sid=$this->core->getParams()['sid'];
	...
	public function sysScript($module, $file, $to=2, $params=array())
	{
