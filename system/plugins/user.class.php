...
class user
{
	...
	public function password_md5($pass, $reg, $time=null, $salt=null)
	{
		if($reg == true)
		{
			...
			$pass_md5 = md5($time . $this->core->conf->salt[0] . $pass . $salt . $this->core->conf->salt[1]);
	...
	//DELETE!!!!
	/*
	public function login($email='', $pass='')//Вход на сайт
	{
		$warning = '';
		$error = '';
		...
		if($email!='' AND $pass!='')
		{
			$email = SanString($email);
			$password = SanString($pass);
			...
			strlen($password)>=$GLOBALS['_CONF']['server']['string_length']['password']['site']['min'])
			{
				...
				if(mysql_num_rows($res)==0)
				{
					...
					if($user)
					{
						$time = $this->time_reg;
						$salt = $this->salt1;
						$pass_md5 = $this->password_md5('site', 'no', $pass, $time, $salt);
						if($pass_md5 == $this->pass)
						{
							$rank = new rank();
							$rank->get_rank($this->id);
							...
							if($rank->warnings<20)
							{
								if($rank->checked==3)
								{
									...
									$this->set_user($user->id, 'id');

									$time_end = time() + 60*60*24*7;
									setcookie('cache_sessid', sha1($this->cache), $time_end, '/', $_SERVER[SERVER_REQUIRE], false/*true*//*);

									return true;
								}
								else
								{
									$warning[] = $_SESSION['WARNING'][] = "Ваш аккаунт не был подтверждён администрацией!";
								}
							}
							else
							{
								$warning[] = $_SESSION['WARNING'][] = "Вы слишком много раз нарушили правила, поэтому были забанены!";
							}
						}
						else
						{
							$warning[] = $_SESSION['WARNING'][] = "Такая пара e-mail, пароль не найдена!";
				...
				else
				{
					...
					$this->email($res['email'], 'activate', array($res['login'], $res['id']));
					$warning[] = $_SESSION['INFO'][] = "Вы не подтвердили свой аккаунт по e-mail. Ключ был выслан повторно. <a href='/login/enter_code/'>Подтвердить аккаунт</a>.";
				}
			}
			else
			{
				$warning[] = $_SESSION['WARNING'][] = "Неправильный формат e-mail и/или пароля!";
			}
		}
		else
		{
			$warning[] = $_SESSION['WARNING'][] = "Вы не ввели e-mail и/или пароль!";
		}
		$array = Array('error'=>$error, 'warning'=>$warning);
		return $array;
	}
	*/
	...
	public function change_user($user, $after, $what)
	{
		...
		if($what=='name')
		{
			if(mb_strlen($after, 'UTF-8')>$this->core->conf->length['name']['max'])
			{
				$this->core->error->error('profile','002');
				return false;
			}
			
			if(mb_strlen($after, 'UTF-8')<$this->core->conf->length['name']['min'])
			{
		...
		if($what=='surname')
		{
			if(mb_strlen($after, 'UTF-8')>$this->core->conf->length['surname']['max'])
			{
				$this->core->error->error('profile','004');
				return false;
			}
			
			if(mb_strlen($after, 'UTF-8')<$this->core->conf->length['surname']['min'])
			{
		...
		if($what=='nickname')
		{
			if(mb_strlen($after, 'UTF-8')>$this->core->conf->length['nickname']['max'])
			{
				$this->core->error->error('profile','006');
				return false;
			}
			
			if(mb_strlen($after, 'UTF-8')<$this->core->conf->length['nickname']['min'])
			{
		...
		if($what=='password' and $after[0]!='')
		{
			...
			if(strlen($after[0])>$this->core->conf->length['password']['max'])
			{
				$this->core->error->error('profile','009');
				return false;
			}
			
			if(strlen($after[0])<$this->core->conf->length['password']['min'])
			{
